...
def test_committed_pilot_cli_preflight_is_network_denied(monkeypatch):
    ...
    assert main(["--plan", "docs/deep_research/e2e/v1.1/glm_e2e_pilot_plan.attempt-003.json", "--task", "single_paper", "--artifact-dir", "outputs/deep_research/e2e/v1/dr-run-5c1662965db19f9b4d4555c1", "--dry-run"]) == 0
...
def test_committed_pilot_freezes_three_distinct_tasks_and_schema(tmp_path: Path):
    plan = parse_e2e_pilot_plan(json.loads(Path("docs/deep_research/e2e/v1.1/glm_e2e_pilot_plan.attempt-003.json").read_text(encoding="utf-8")))
