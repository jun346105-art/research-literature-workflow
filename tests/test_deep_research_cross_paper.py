...
from litflow.deep_research.e2e import E2ETerminalError, GLME2ECrossPaperAttemptPlan, GLMSingleWriter, DeepResearchRunner, _validate_cross_paper_allowlist, parse_e2e_pilot_plan, preflight_e2e_pilot, prompt_hashes, require_cross_paper_comparison, runtime_source_sha256, task_input_sha256
...
def test_source_scope_is_applied_before_bm25_ranking():
    full = _full_corpus()
    registry = ReadOnlyToolRegistry(full, allowed_source_keys=("L4DLHQUZ",))
    hits = asyncio.run(registry.invoke(ToolName.search_local_corpus, LocalSearchRequest(query="retrieve explicitly grounded method descriptions from local source L4DLHQUZ", top_k=12)))
    target = "L4DLHQUZ:L4DLHQUZ_chunk_0007"
    assert target in {hit.passage_id for hit in hits}
    assert next(hit.rank for hit in hits if hit.passage_id == target) == 11
    assert registry.last_search_stats == {"pre_filter_candidate_count": len(full), "source_scoped_candidate_count": 16, "source_scoped_result_count": 12, "bounded_top_k": 12}


def test_executor_source_scoped_retrieval_accepts_bounded_qrels(tmp_path: Path):
    task, brief, approval, plan, _, _, spec, _ = _inputs(tmp_path)
    registry = ReadOnlyToolRegistry(_full_corpus())
    routing = {plan.subtasks[0].subtask_id: ("L4DLHQUZ",), plan.subtasks[1].subtask_id: ("3NLKTSIP",)}
    allowed_passages = ("L4DLHQUZ:L4DLHQUZ_chunk_0007", "3NLKTSIP:3NLKTSIP_chunk_0005")
    result = asyncio.run(LocalResearchExecutor(registry, budget=spec).execute(task, brief, approval, plan, event_path=tmp_path / "scoped.jsonl", checkpoint_path=tmp_path / "scoped.checkpoint.json", run_id="dr-run-cross-scoped", allowed_source_keys=("L4DLHQUZ", "3NLKTSIP"), allowed_passage_ids=allowed_passages, allowed_source_keys_by_subtask=routing, retrieval_top_k=12))
    assert {unit.locator.passage_id for unit in result.evidence_graph.evidence_units} == set(allowed_passages)
    assert {source.bibliographic_metadata["paper_key"] for source in result.evidence_graph.sources} == {"L4DLHQUZ", "3NLKTSIP"}
...
def test_executor_known_failure_is_terminalized_by_e2e_runner(tmp_path: Path):
    ...
    planner.last_source_refs = {(draft.subtasks[0].question, draft.subtasks[0].rationale): "L4DLHQUZ"}
    runner = DeepResearchRunner(planner, LocalResearchExecutor(registry, budget=spec), writer, budget=spec, comparison_required=True, allowed_source_keys=("L4DLHQUZ", "3NLKTSIP"), allowed_passage_ids=("L4DLHQUZ:L4DLHQUZ_chunk_0007", "3NLKTSIP:3NLKTSIP_chunk_0005"), retrieval_top_k=1)
