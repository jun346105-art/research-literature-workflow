...
def _plans():
    ...
    return [parse_paired_plan(json.loads(path.read_text(encoding="utf-8"))) for path in sorted(PAIR_DIR.glob("paired_*attempt-003.json"))]
...
def test_paired_cli_dry_run_preflight_uses_no_key_or_network(tmp_path):
    ...
    env["PYTHONPATH"] = "src"
    for path in sorted(PAIR_DIR.glob("paired_*attempt-003.json")):
