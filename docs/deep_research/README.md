...
- 当前 Session：`B08E2E-R6C Writer Calibration CLI Wiring`（`internal_result`；仅离线校准，未执行真实 Writer）
- 当前简历状态：`not_ready`
