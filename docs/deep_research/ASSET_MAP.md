...
- **S02 closure:** NumPy is now declared for default runtime and locked at `2.5.2`; PyMuPDF `1.28.2` is a test extra because production PDF extraction uses `pypdf` and `fitz` occurs only in Reading Context tests. `torch` and `transformers` remain delayed Dense `_Encoder` dependencies, not established MVP runtime requirements. The S01 snapshot remains unchanged in `asset_inventory.json`.
