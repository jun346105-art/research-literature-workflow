...
def build_structured_reading_prompt(llm_input: dict) -> str:
    ...
    return (
        ...
        "- evidence_text must be copied verbatim from the cited chunk text; preserve PDF extraction artifacts and line-break wording.\n"
        "- Keep each evidence_text as a short contiguous source span, preferably under 200 characters.\n"
        "- Do not fix typos, spacing, hyphenation, ligatures, or broken words in evidence_text.\n"
        "- Do not use evidence_text that crosses pages, chunks, page headers, footers, tables, or figure captions.\n"
        "- If you cannot verify a quote is inside the same chunk_id, omit that evidence_link instead of guessing.\n"
