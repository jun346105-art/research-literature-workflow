...
class TranslationResponse(BaseModel):
    ...
    def validate_against_query(self, query: dict[str, Any]) -> None:
        ...
        missing_entities = [entity for entity in entities if entity.casefold() not in translated or entity.casefold() not in {item.casefold() for item in self.preserved_entities}]
        if missing_entities:
            raise TranslationContractError("entity preservation failed: " + ", ".join(missing_entities))
        numbers = _numbers_and_units(query["query_zh"])
        missing_numbers = [item for item in numbers if item.casefold() not in translated or item.casefold() not in {value.casefold() for value in self.preserved_numbers_and_units}]
        if missing_numbers:
...
def run_query_translation(queries_path: Path, run_dir: Path, *, model: str, query_ids: list[str], client: Any | None = None, resume: bool = False) -> dict[str, Any]:
    ...
    for query_id in query_ids:
        ...
        try:
            ...
            result = {"query_id": query_id, "execution_status": "success", "translation": response.model_dump(), "raw_response_artifacts": raw_paths, "validation_error": None, **item}
        except TranslationContractError as exc:
            result = {"query_id": query_id, "execution_status": "contract_failed", "translation": None, "raw_response_artifacts": raw_paths, "validation_error": str(exc), **item}
        except Exception as exc:
            result = {"query_id": query_id, "execution_status": "provider_or_parse_failed", "translation": None, "raw_response_artifacts": raw_paths, "validation_error": str(exc), **item}
...
def _protected_entities(query_zh: str) -> list[str]:
    return list(dict.fromkeys(re.findall(r"[A-Za-z][A-Za-z0-9-]*", query_zh)))


def _numbers_and_units(query_zh: str) -> list[str]:
    return list(dict.fromkeys(re.findall(r"\d+(?:\.\d+)?(?:%|°|cm|FPS)?", query_zh, flags=re.IGNORECASE)))
